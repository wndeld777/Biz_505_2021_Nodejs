// fetch 를 통해서 되돌려 받은 주문리스트를 왼쪽 주문리스트에 표현하기
const add_order_list = (menu_list) => {
  ...
  // div.order_list 를 가져와서, 전체를 article.order_list 로부터 삭제하기
...
// fetch 를 사용하여 서버에 데이터를 요청하기 위해 별도의 함수를 선언하기
const order_input = (table_id, menu_id) => {
  // path Varriable 방식으로 menu_id 값을 URL 에 포함하여
  // getter 요청하기

  /**
   * 만약 3번 테이블에 5번 메뉴를 추가하려고 Request 를 한다면
   *  localhost:3000/order/3/input/5 와 같은 URL 을 만들어 서버로 Request한다
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (product_article) {
    product_article.addEventListener("click", (e) => {
      ...
      // index.pug 의 table layout click 설정
