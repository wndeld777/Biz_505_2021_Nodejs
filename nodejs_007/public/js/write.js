...
$(function () {
  $("#b_text").summernote({
    ...
    callbacks: {
	   ...
	   * 
	   * summernote 입력창에
	   * 이미지를 d&drop 을 하면 입력창에 바로
	   * 이미지를 추가하면서 글을 작성할 수 있다
	   * 
	   * 이런식으로 이미지를 추가하면
	   * 작성되는 글 내용에 이미지가 encoding 된 코드로 변경이 되고
	   * 글 내용의 크기가 어마어마하게 커져 버린다
	   * DB 에 해당 내용을 저장하려면 
	   * BLOB, CLOB type 으로 칼럼으로 만들고
	   * 저장하는 방법이 있다
	   * 하지만,
	   * 일반적으로 Web Server App 에서는
	   * 한꺼번에 대량의 text 를 업로드 하는 것을 허용하지 않는다
	   * 또한 BLOB, CLOB type 의 칼럼을 가진 table 은
	   * 굉장히 용량이 커져서 무거워지고
	   * text 검색등을 하기가 어려워 진다
	   * 
	   * MySQL에서는 가급적 BLOB 등 보다는 
	   * Text type 이나 VARCHAR(4000) 와 같은 
	   * type 이하로만 칼럼을 설정할 것을 권장한다
	   * 
	   * 때문에 이미지가 포함된 데이터는 실제로 서버로 업로드가 안되고
	   * DB Table 에도 저장할 수 없다고
	   * 생각하고 App을 개발, 설계 하는것이 좋다
	   * 
	   ...
	   * 
	   * summernote 에 onImageUpload 이벤트를 설정하여
	   * note 창에 이미지를 올리면
	   * 가로채서 ajax로 이미지를 먼저 올리도록 custermizing 을 수행한다
	   * 
