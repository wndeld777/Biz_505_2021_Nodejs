...
/**
 * promise
 * 지금까지 사용된 JS 코드는 대부분이 비동기 방식이고
 * 코드(함수)들의 순서가 보장되지 않았다
 * 순서대로 작성된 코드가 지 맘대로 실행되기 때문에
 * 그 부분을 해결하기 위해 call back 이라는 개념으로 코딩을 했다
 *
 * call back 이 계속 겹치는 상황이 발생할 수 있고
 * 그러다보면 가독성이 매우 떨어지는 코드가 된다
 * a(call()=>{
 * 	b(call()=>{
 * 	})
 * })
 * JS 에서 이러한 상황을 Callback Hell
 * async await
 * */
router.get("/order/:table_id/input/:menu_id", async (req, res) => {
  ...
  // 선택된 상품이 order list 에 있는지를 검사하기 위해서
