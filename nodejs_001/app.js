/**
 * nodejs 프로젝트에서는 많은 외부 모듈을 import 하여
 * 사용하게 된다.
 * import 된 모듈을 직접 호출하여 기능을 구현하거나
 * 또는 다른 모듈과 협력하여 background 에서 작동되기도 한다
 *
 * 이처럼 모듈을 import 하면 프로젝트에서는 이들을 middle ware 가 설정(치)되었다 라고 한다.
 *
 */

var createError = require("http-errors");
// web application 에서 사용자의 request URL 을 처리하고
// respnse하는 역할을 수행하는 middle ware 이며, framework 이다
...
// 쉽게 지정을 할 수 있다
// 지금 현재는 운영체제와 무관하게 path(폴더 경로)를
// 슬래시(/)를 사용하지만
// 과거에 윈도우에서는 역슬래시(\)를 사용하고
//    Unix(Linux, mac) 등에서는 슬래시(/) 사용하여야 만 했던 시절이 있다
//    이때부터 path 와 같은 도구를 사용하여
