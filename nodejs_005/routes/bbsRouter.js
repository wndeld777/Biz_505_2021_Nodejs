...
// 설정된 /write get는
// URL 에서 localhost:3000/bbs/write 요청할때
// 응답할 함수
router.get("/", (req, res) => {
  const BBS = {
...
router.get("/detail", (req, res) => {
  // list 에서 게시물을 클릭했을때
  // 게시물의 id(b_id) 값을 queryString 으로 가지고 여기에 도달한다
  const b_id = req.query.b_id;

  // PK 를 기준으로 1개의 데이터를 추출하라
...
router.get("/delete", (req, res) => {
  ...
  tbl_bbs
    // 데이터를 삭제하라
    .distroy({
      // b_id 칼럼의 값이 변수 b_id 에 담긴 값과 같으면
      where: { b_id },
    })
    .then(() => {
      res.redirect("/");
