 ...
 * JS 에서 의 배열
 * 배열요소의 타입이 고정이 아니다
 *      다른 type 의 요소를 같은 배열에 저장 할 수 있다
 */
// JS 에서 배열을 만들기
// blacnk 배열
let 배열1 = [];
 ...
 *      return 문이 5번 실행 될것이다
 * 3. 또다른배열.push(return 된 결과)의 코드가 5번 실행된다
 *
 * 이 결과는 배열3에 담긴 모든 요소에 끝에 문자열 A 가 부착되고
 * 문자열 A 가 부착된 요소들이 또다른배열에 PUSH 되어 새로운 배열이 만들어 진다
 */
...
// map 은 내부의 함수에 요소와 배열의 index 값을 매개변수로 전달 할 수 있고
// 내부 코드에서는 전달받은 요소와 index 를 활용하여 코드를 작성할 수 있다
숫자배열.map((숫자, index) => {
...
// 다른 배열로 만들고 싶을때
// 내부코드에서 특정 요소가 연산될때
// 마지막에 return 이 true 인 경우만 해당요소를 다른 배열에 push
